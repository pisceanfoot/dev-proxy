# Spec: Named Upstreams

## Purpose

Defines a top-level `upstreams` map in `dev-proxy.yaml` that allows upstream targets to be named and reused across routes, and an `upstream_path` field that rewrites the forwarded path when a named upstream is used.
