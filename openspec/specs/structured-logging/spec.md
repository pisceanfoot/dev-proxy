...
Defines how dev-proxy controls its log verbosity via a `log_level` field in `dev-proxy.yaml`, and what output each level produces at startup, during request handling, and on config reload.
