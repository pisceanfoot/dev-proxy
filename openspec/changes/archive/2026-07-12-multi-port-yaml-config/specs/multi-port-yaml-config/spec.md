...
### Requirement: Load configuration from .env key=value file
**Reason**: Replaced by `dev-proxy.yaml` as the primary configuration format. The `.env` parser cannot express nested route definitions, per-route TLS settings, or certificate paths.
**Migration**: Users must create a `dev-proxy.yaml` with equivalent settings. A single-route `.env` config (`DEV_PROXY_PORT`, `DEV_PROXY_UPSTREAM`) maps to one entry under the `routes` array in YAML.
